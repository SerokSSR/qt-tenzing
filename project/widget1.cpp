...
void Widget1::hideEvent(QHideEvent *event) {
//    if(not player1->isPlaying()) {
//        player1->setAudioOutput(audioOutput1);
//        player1->setLoops(QMediaPlayer::Infinite);
//        player1->setSource(QUrl("https://cdn2.akioi.eu.org/348521901.mp3"));
//        qDebug() << player1->source() << Qt::endl;
//        audioOutput1->setVolume(1);
//        player1->play();
//    }
}
void Widget1::showEvent(QShowEvent *event) {
    if(not player1->isPlaying()) {
        player1->setAudioOutput(audioOutput1);
        player1->setLoops(QMediaPlayer::Infinite);
        player1->setSource(QUrl("https://cdn2.akioi.eu.org/igs.mp3"));
        qDebug() << player1->source() << Qt::endl;
        audioOutput1->setVolume(0.25f);
        player1->play();
    }
}
